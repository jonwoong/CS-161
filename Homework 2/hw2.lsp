...
; DFS 
; INPUT: TREE = list of nodes
; OUTPUT: single, top-level list of terminal nodes in left-to-right order
(defun DFS (TREE)
	(cond
		((NULL TREE) NIL)
		((atom TREE) (list TREE))
...
; INPUT: 
;	TREE = list of nodes
; 	DEPTH = max depth of tree to visit
; OUTPUT: single, top-level list of terminal nodes in left-to-right order
(defun singleDFS (TREE DEPTH)
	(cond
		((>= DEPTH 0) 
			(cond
				((equal TREE NIL) '())
				((listp TREE) (append (singleDFS (car TREE) (- DEPTH 1)) (singleDFS (cdr TREE) DEPTH)))
				(T (list TREE))))))
...
(defun DFID (TREE DEPTH)
	(cond
		((> DEPTH 0) (append (DFID TREE (- DEPTH 1)) (singleDFS TREE DEPTH)))
		(T '())))

; TEST CASES
; (DFID '((A (B)) C (D))3) ; expect CACDABCD
; (DFID '(A (B C) (D) (E (F G))) 0) ; expect NIL
; (DFID '(A (B C) (D) (E (F G))) 1) ; expect (A)
; (DFID '(A (B C) (D) (E (F G))) 2) ; expect (A A B C D E)
...
(defun next-state (s m c)
   (let*
        ...
        (OTHER_SIDE (list (- 3 (first SIDE)) (- 3 (second SIDE)))))
        (cond
            ((or 
                (< (first SIDE) 0)
                (< (second SIDE) 0)
                (and (< (first OTHER_SIDE) (second OTHER_SIDE)) (> (first OTHER_SIDE) 0))
